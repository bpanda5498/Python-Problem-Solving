...
def segment_with_active_contour(image, seed_point_variation=0):
    ...
    left_x_center = 140 + seed_point_variation  # Example value, introduce variation (optional)
    left_y_center = 150
    left_r = 50
    left_snake_init = np.array([[left_x_center + left_r * np.cos(t), left_y_center + left_r * np.sin(t)]
    ...
    right_x_center = 140 + seed_point_variation  # Example value, introduce variation (optional)
    right_y_center = 160
    right_r = 50
    right_snake_init = np.array([[right_x_center + right_r * np.cos(t), right_y_center + right_r * np.sin(t)]
                                  for t in np.linspace(0, 2 * np.pi, 100)])
